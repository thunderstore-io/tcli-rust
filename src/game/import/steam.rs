...
impl SteamImporter {
    pub fn new(appid: &str) -> Self {
        SteamImporter {
            steam_dir: None,
            appid: appid.parse::<u32>().expect("Got a bad appid from the ecosystem schema. This is a bug"),
        }
    }
...
impl GameImporter for SteamImporter {
    fn construct(self: Box<Self>, base: ImportBase) -> Result<GameData, Error> {
            ...
            None => {
                let steam = self
                    ...
                    .map_err(|e: steamlocate::Error| match e {
                        steamlocate::Error::InvalidSteamDir(_) => Error::SteamDirBadPath(self.steam_dir.as_ref().unwrap().to_path_buf()),
                ...
                let (app, lib) = steam
                    .find_app(self.appid) 
                    .unwrap_or_else(|e| panic!("An error occured while searching for app with id '{}': {e:?}.", self.appid))
                    .ok_or_else(|| Error::SteamAppNotFound(self.appid, steam.path().to_path_buf()))?;
        ...
        let exe_path = r2mm
            ...
            .find(|x| x.is_file())
            .ok_or_else(|| super::Error::ExeNotFound(base.game_def.label.clone(), app_dir.clone()))?;
